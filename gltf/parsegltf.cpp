...
static void parseMeshInstancesGltf(std::vector<Transform>& instances, cgltf_node* node)
{
	...
	for (size_t i = 0; i < node->mesh_gpu_instancing.attributes_count; ++i)
	{
		...
		if (strcmp(attr.name, "TRANSLATION") == 0 && attr.data->type == cgltf_type_vec3)
			...
			scale = attr.data;
		else if (strcmp(attr.name, "_COLOR_0") == 0 && attr.data->type == cgltf_type_vec3) {
			streams.push_back(Stream());
			Stream& s = streams.back();

			s.type = cgltf_attribute_type_color;
			s.index = attr.index;
			s.custom_name = attr.name;
		...
		else if (attr.type == cgltf_attribute_type_custom && isIdAttribute(attr.name))
		{
			streams.push_back(Stream());
			Stream& s = streams.back();
	...
	for (size_t i = 0; i < count; ++i)
	{
		...
		{
			const Stream& s = streams[j];
			if (s.data.size() != count)
				continue;

			xf.custom_atts.push_back(Stream());
			Stream& custom = xf.custom_atts.back();
			custom.type = s.type;
			custom.index = s.index;
			custom.custom_name = s.custom_name;
			custom.data.resize(1);
			custom.data[0] = s.data[i];
		}
